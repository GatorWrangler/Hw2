...
function begin_play(){
	...
	if(isEmpty(p1.value) || isEmpty(p2.value)){
		alert("Player names can not be empty");
		return;
	}
	p1.value += " (X)";
	p2.value += " (O)";
	...
	p2.disable = true;
	this.started = true;
	document.getElementById("turn_info").value = "Turn: " + p1.value;
	document.getElementById("turn_info").innerText = "Turn: " + p1.value;
}
...
function reset_play(){
	...
	p1.disable = false;
	p2.disable = false;
	document.getElementById("turn_info").innerText = "Game has not stated";
}
...
function play() {
	...
	var moveTxField = document.getElementById("move_text_id");
	if(isEmpty(moveTxField.value)){
		alert("Move can not be empty");
		return;
	}
	var regEx = /[A-C][1-3]/g;
	var value = moveTxField.value.toUpperCase();
	var move = moveTxField.value.toUpperCase().match(regEx);

	if(!move){
		alert("That is an Invalid Move");
		return;
	}
	if(document.getElementById(value).innerText == "X" || document.getElementById(value).innerText == "O"){
		alert("That is an Invalid Move");
		return;
	}
	if(turn == true){
	...
	switch(value){
		case "A1":
			board_state[0] = checker;
			break;
		case "A2":
			board_state[1] = checker;
			break;
		case "A3":
			board_state[2] = checker;
			break;
		case "B1":
			board_state[3] = checker;
			break;
		case "B2":
			board_state[4] = checker;
			break;
		case "B3":
			board_state[5] = checker;
			break;
		case "C1":
			board_state[6] = checker;
			break;
		case "C2":
			board_state[7] = checker;
			break;
		case "C3":
			board_state[8] = checker;
			break;
		default:
			break;
	}
...
function checkWin(){
	...
	if ((board_state[0] == checker) && (board_state[1] == checker) && (board_state[2] == checker)) {
        winner(checker);
    } else if ((board_state[0] == checker) && (board_state[4] == checker) && (board_state[8] == checker)) {
        winner(checker);
    } else if ((board_state[0] == checker) && (board_state[3] == checker) && (board_state[6] == checker)) {
        winner(checker);
    } else if ((board_state[1] == checker) && (board_state[4] == checker) && (board_state[7] == checker)) {
        winner(checker);
    } else if ((board_state[2] == checker) && (board_state[5] == checker) && (board_state[8] == checker)) {
        winner(checker);
    } else if ((board_state[3] == checker) && (board_state[4] == checker) && (board_state[5] == checker)) {
        winner(checker);
    } else if ((board_state[6] == checker) && (board_state[7] == checker) && (board_state[8] == checker)) {
        winner(checker);
    } else if ((board_state[6] == checker) && (board_state[4] == checker) && (board_state[2] == checker)) {
        winner(checker);
    }
}
